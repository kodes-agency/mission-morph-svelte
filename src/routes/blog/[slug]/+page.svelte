          ...
          <div class="relative">
            ...
            <div bind:this={buttonShareWrapperEl} class="absolute invisible left-28 -top-6 md:top-auto md:left-auto flex mt-2 md:flex-col space-x-2 md:space-x-0 md:space-y-2 bg-light-cyan md:mt-2 p-4 border rounded-3xl">
              <div class="flex space-x-2">
                ...
                <a target="_blank" href={`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(window.location)}&title=${encodeURIComponent(data.data.blogs.data[0].attributes?.seo?.title)}&source=${encodeURIComponent('Mission Morph')}`} class="border p-2 tiny bg-white rounded-full flex items-center justify-center hover:scale-95 transition-all duration-200">
              ...
              <button on:click={copyContent} class="border tiny bg-white p-1 {isCopied ? "bg-green text-white" : ""} text-sm rounded-full flex items-center justify-center hover:scale-95 transition-all duration-200">
