          ...
          <a aria-label="{service.attributes?.homePageCursorLable} page" href="/service/{service.attributes?.slug}" class="pointer-events-none lg:pointer-events-auto">
            <img
              ...
              alt={service.attributes?.homePageThumbnail?.data?.attributes
                ?.alternativeText}
              class="img interactable w-60 md:w-auto md:h-[70vh] 2xl:h-[60vh] max-w-xs object-cover absolute -top-40 right-0 md:left-40 lg:left-80 z-0"
              data-labelBold="explore"
              data-label={service.attributes?.homePageCursorLable}
              data-id={i}
            />
      ...
      <div class="h-40 relative" data-speed="1.05">
        {#each services as service, i}
          <div class="text z-10 absolute" data-id={i}>
            ...
            <a class="border-b-2 lg:hidden border-medium-purple tiny text-light-cyan uppercase hover:scale-95 transition-transform duration-200 text-xl font-bold" aria-label="{service.attributes?.homePageCursorLable} page" href="/service/{service.attributes?.slug}">Read more about {service.attributes?.homePageCursorLable}</a>
          </div>
        {/each}
      </div>
    </div>
