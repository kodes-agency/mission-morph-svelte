...
<section class="pt-10 bg-gradient-to-b from-dark-purple to-{color}-bg-color min-h-screen flex flex-col items-center justify-center relative">
    <img {src} {alt} class="absolute top-0 left-0 h-full w-full object-cover z-0 opacity-20">
    <div class="flex flex-col justify-between h-full max-w-5xl relative z-10 py-10">
        <div></div>
        <div class="flex flex-col space-y-10 p-6">
            ...
            <h1 class="text-3xl md:text-5xl lg:text-8xl leading-[1.2] font-black text-white">{heading}</h1>
            <p class="text-white md:text-lg max-w-3xl">{content}</p>
        </div>
        <p class=" text-{color}-text-color underline text-sm w-full text-center">{scroll}</p>
    </div>
</section>
