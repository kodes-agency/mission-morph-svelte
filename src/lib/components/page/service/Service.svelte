<script lang="ts">
  ...
  export let color: string | undefined = "seo-text-color";
...
<section
  class=" min-h-screen bg-black flex flex-col pl-[15vw] 2xl:pl-[20vw] p-40"
>
  <Accordion
    ...
    {#each subservice as service, i}
      <AccordionItem open={i === 0}>
        <svelte:fragment slot="title">
          <div class="w-full h-px bg-white my-10" />
          <h2
            class=" text-white font-bold text-start text-4xl md:text-5xl transition-all duration-700 mb-4"
          >
            {service.title}
          </h2>
        </svelte:fragment>
        ...
        <svelte:fragment slot="footer">
          <p class="text-{color}-text-color">{service.price}</p>
        </svelte:fragment>
      </AccordionItem>
    {/each}
  </Accordion>
